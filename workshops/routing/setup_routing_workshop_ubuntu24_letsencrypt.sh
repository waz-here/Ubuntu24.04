...
#   - Optional Let's Encrypt certificate using Certbot HTTP-01 validation.
#   - Optional locked-down instructor LXC jump host on 192.168.30.222/24.
#
# Run:
...
Options:
  ...
  --letsencrypt           Request and configure a Let's Encrypt certificate.
                          Uses HTTP-01 validation through Nginx on TCP/80.
  --domain NAME_OR_URL    Public DNS name or URL for the browser console.
  --letsencrypt-email EMAIL
                          Optional Let's Encrypt account contact email.
  ...
  --instructor-authorized-keys FILE
                          Install one or more instructor SSH public keys.
  --instructor-password
                          Prompt securely for an instructor password.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --domain)
            ...
            ;;
        --letsencrypt-email)
            [[ $# -ge 2 ]] || die "--letsencrypt-email requires a value"
            LETSENCRYPT_EMAIL="$2"
            shift 2
            ;;
        --enable-firewall)
...
normalise_letsencrypt_domain() {
    [[ "$ENABLE_LETSENCRYPT" -eq 1 ]] || return

    [[ "$INSTALL_GUACAMOLE" -eq 1 ]] || \
        die "--letsencrypt requires Guacamole/Nginx."

    [[ -n "$LETSENCRYPT_DOMAIN" ]] || \
        die "--letsencrypt requires --domain NAME_OR_URL."

    LETSENCRYPT_DOMAIN="${LETSENCRYPT_DOMAIN#http://}"
    LETSENCRYPT_DOMAIN="${LETSENCRYPT_DOMAIN#https://}"
    LETSENCRYPT_DOMAIN="${LETSENCRYPT_DOMAIN%%/*}"
    LETSENCRYPT_DOMAIN="${LETSENCRYPT_DOMAIN%%:*}"
    LETSENCRYPT_DOMAIN="${LETSENCRYPT_DOMAIN%.}"
    LETSENCRYPT_DOMAIN="${LETSENCRYPT_DOMAIN,,}"

    [[ -n "$LETSENCRYPT_DOMAIN" ]] || die "Invalid empty --domain value."

    if [[ ! "$LETSENCRYPT_DOMAIN" =~ ^([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}$ ]]; then
        die "Invalid public DNS name for --domain: ${LETSENCRYPT_DOMAIN}"
    fi

    if [[ -n "$LETSENCRYPT_EMAIL" ]] && \
       [[ ! "$LETSENCRYPT_EMAIL" =~ ^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$ ]]; then
        die "Invalid --letsencrypt-email value: ${LETSENCRYPT_EMAIL}"
    fi

    log "Let's Encrypt requested for ${LETSENCRYPT_DOMAIN}."
}
...
echo
echo "Starting Dynagen inside detached screen session '\${SCREEN_NAME}'."
screen -dmS "\$SCREEN_NAME" bash -lc "cd '\$ROUTING_DIR' && exec dynagen topology.net"
sleep 2

if screen -list | grep -q "[.]\${SCREEN_NAME}[[:space:]]"; then
    echo
    echo "Dynagen is running."
    echo "Attach with: screen -r \${SCREEN_NAME}"
    echo "At the Dynagen prompt run: start /all"
else
    echo "ERROR: Dynagen did not remain running."
    echo "Troubleshoot with: cd '\$ROUTING_DIR' && dynagen topology.net"
    exit 1
fi
EOF
...
configure_nginx() {
    ...
    if [[ "$ENABLE_LETSENCRYPT" -eq 1 ]]; then
        ...
        log "Configuring Nginx for ${LETSENCRYPT_DOMAIN} before Let's Encrypt validation."
    else
    ...
    local server_name="_"
    [[ "$ENABLE_LETSENCRYPT" -eq 1 ]] && server_name="$LETSENCRYPT_DOMAIN"
...
EOF
    ...
    if [[ "$ENABLE_LETSENCRYPT" -eq 0 ]]; then
        log "Nginx configured for HTTPS browser access using the self-signed certificate."
        return
    fi

    log "Checking DNS resolution for ${LETSENCRYPT_DOMAIN}."
    local resolved_v4 resolved_v6
    resolved_v4="$(getent ahostsv4 "$LETSENCRYPT_DOMAIN" 2>/dev/null | awk '{print $1}' | sort -u | paste -sd, - || true)"
    resolved_v6="$(getent ahostsv6 "$LETSENCRYPT_DOMAIN" 2>/dev/null | awk '{print $1}' | sort -u | paste -sd, - || true)"

    if [[ -z "$resolved_v4" && -z "$resolved_v6" ]]; then
        die "DNS for ${LETSENCRYPT_DOMAIN} does not resolve. Create the DNS record and rerun."
    fi

    [[ -n "$resolved_v4" ]] && log "DNS IPv4: ${resolved_v4}"
    [[ -n "$resolved_v6" ]] && log "DNS IPv6: ${resolved_v6}"
    ...
    if [[ -n "$LETSENCRYPT_EMAIL" ]]; then
        contact_args=(--email "$LETSENCRYPT_EMAIL")
    else
        contact_args=(--register-unsafely-without-email)
        log "No Let's Encrypt contact email supplied."
    fi

    log "Requesting Let's Encrypt certificate for ${LETSENCRYPT_DOMAIN} using HTTP-01."
    log "TCP/80 must be publicly reachable and DNS must point to this server."
    ...
    [[ -f "/etc/letsencrypt/live/${LETSENCRYPT_DOMAIN}/fullchain.pem" ]] || \
        die "Certbot completed but the expected certificate was not found."

    log "Let's Encrypt certificate installed for ${LETSENCRYPT_DOMAIN}."
}
...
configure_instructor_nat() {
    ...
    [[ -n "$WAN_INTERFACE" ]] || die "Could not determine WAN interface."

    log "Instructor jump host WAN interface detected as ${WAN_INTERFACE}."

    while iptables -t nat -C PREROUTING -i "$WAN_INTERFACE" -p tcp \
        --dport "$INSTRUCTOR_SSH_WAN_PORT" \
        -j DNAT --to-destination "${INSTRUCTOR_JUMPHOST_IP}:22" 2>/dev/null; do
        iptables -t nat -D PREROUTING -i "$WAN_INTERFACE" -p tcp \
            --dport "$INSTRUCTOR_SSH_WAN_PORT" \
            -j DNAT --to-destination "${INSTRUCTOR_JUMPHOST_IP}:22"
    done

    while iptables -t nat -C POSTROUTING -s "$INSTRUCTOR_NETWORK" \
        -o "$WAN_INTERFACE" -j MASQUERADE 2>/dev/null; do
        iptables -t nat -D POSTROUTING -s "$INSTRUCTOR_NETWORK" \
            -o "$WAN_INTERFACE" -j MASQUERADE
    done

    iptables -t nat -A PREROUTING -i "$WAN_INTERFACE" -p tcp \
        --dport "$INSTRUCTOR_SSH_WAN_PORT" \
        -j DNAT --to-destination "${INSTRUCTOR_JUMPHOST_IP}:22"
    iptables -t nat -A POSTROUTING -s "$INSTRUCTOR_NETWORK" \
        -o "$WAN_INTERFACE" -j MASQUERADE
    ...
    iptables -I FORWARD 1 -i "$INSTRUCTOR_BRIDGE" -o "$WAN_INTERFACE" \
        -s "$INSTRUCTOR_JUMPHOST_IP" -p tcp \
        -m multiport --dports 53,80,443 -j ACCEPT
    iptables -I FORWARD 1 -i "$WAN_INTERFACE" -o "$INSTRUCTOR_BRIDGE" \
...
configure_instructor_jumphost() {
    ...
    if ! lxc-info -n "$INSTRUCTOR_CONTAINER" >/dev/null 2>&1; then
        ...
        case "$(dpkg --print-architecture)" in
            ...
            *) die "Instructor LXC supports amd64 and arm64 hosts only." ;;
        esac
        lxc-create -n "$INSTRUCTOR_CONTAINER" -t download -- \
            -d ubuntu -r noble -a "$lxc_arch" >>"$LOG_FILE" 2>&1
    else
...
EOF
    ...
    if [[ "$PROMPT_INSTRUCTOR_PASSWORD" -eq 1 || "$instructor_keys_enabled" -eq 0 ]]; then
        ...
        while true; do
            ...
            [[ "$instructor_password" == "$instructor_password_confirm" ]] || {
                echo "Passwords do not match. Try again." >&2
                continue
            }
            [[ ${#instructor_password} -ge 12 ]] || {
                echo "Instructor password must contain at least 12 characters." >&2
                continue
            }
            break
        done
    ...
    if [[ "$instructor_keys_enabled" -eq 1 ]]; then
        install -d -m 0700 "${rootfs}/home/${INSTRUCTOR_USER}/.ssh"
...
while true; do
    ...
    case "$choice" in
        q|Q) exit 0 ;;
        ''|*[!0-9]*) echo "Invalid selection."; sleep 1 ;;
        *)
...
configure_iptables() {
    ...
    if [[ "$ENABLE_INSTRUCTOR_JUMPHOST" -eq 1 ]]; then
        iptables -A INPUT -i "$INSTRUCTOR_BRIDGE" \
            -s "$INSTRUCTOR_JUMPHOST_IP" -d "$INSTRUCTOR_HOST_IP" \
            -p tcp --dport 3001:3014 -j ACCEPT
    fi
...
validate_installation() {
    ...
    if [[ "$ENABLE_LETSENCRYPT" -eq 1 ]]; then
        [[ -f "/etc/letsencrypt/live/${LETSENCRYPT_DOMAIN}/fullchain.pem" ]] ||             die "Let's Encrypt certificate validation failed."
    fi
...
display_summary() {
    ...
    if [[ "$INSTALL_GUACAMOLE" -eq 1 ]]; then
        ...
        if [[ "$ENABLE_LETSENCRYPT" -eq 1 ]]; then
            ...
            echo "  Let's Encrypt certificate for ${LETSENCRYPT_DOMAIN}"
        else
